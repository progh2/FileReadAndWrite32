...
namespace FileReadAndWrite
{
    public partial class FormFile : Form
    {
        ...
        {
            if(this.txtReadFile.Text == "")
            {
                MessageBox.Show("읽을 파일을 선택해 주세요", "에러", 
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            if (!File.Exists(this.txtReadFile.Text))
            {
                MessageBox.Show("파일이 존재하지 않습니다.", "에러",
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
